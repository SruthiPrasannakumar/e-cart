...
const Header = ({insideHome}) => {
  ...
  return (
    <nav className='flex bg-violet-600 fixed w-full p-5 text-white font-bold'>
        <Link className='text-2xl ' to={'/'}><i className='fa-solid fa-truck-fast me-1'></i>Daily Cart
        </Link>
        <ul className='flex-1 text-right'>
            { insideHome && <li className='list-none inline-block px-5'><input style={{width:'300px'}} className='rounded p-1 text-black' type="text" onChange={e=>dispatch(searchProduct(e.target.value.toLowerCase()))} placeholder='Search Products here!' /></li>}
